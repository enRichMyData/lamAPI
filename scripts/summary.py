...
def enhance_and_store_results(
    db_name,
    collection_name,
    summary_collection_name,
    pipeline,
    label_resolver_collection,
):
    db = client[db_name]
            ...
            buffer.append(
                {
                    ...
                    "countNormMinMax": round(
                        (result["count"] - distribution_min)
                        / (distribution_max - distribution_min),
                        2,
                    ),
                    "countNormZScore": round(
                        (result["count"] - distribution_mean) / distribution_stdev, 2
                    ),
                }
            )
        else:
