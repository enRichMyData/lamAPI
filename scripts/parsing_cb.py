    ...
    for collection, fields in index_specs.items():
        if collection == "cache":
            db[collection].create_index(
                [(field, 1) for field in fields], unique=True, background=True
            )
...
def parse_data(index, columns, data, additional_data):
    ...
    all_aliases = {
        "en": [
            str(data.get(f"alias{i}"))
            for i in range(1, 4)
            if data.get(f"alias{i}") and str(data.get(f"alias{i}")) != "nan"
        ]
    }
    ...
    join = {
        ...
        "literals": {"id_entity": index, "entity": entity, "literals": literals},
        "types": {"id_entity": index, "entity": entity, "types": types},
    }
...
def main():
    ...
    c_ref = {"items": db.items, "literals": db.literals, "types": db.types}

    create_indexes(db)

    buffer = {"items": [], "literals": [], "types": []}
